...
q=ti.Vector.field(2,dtype=ti.f32,shape=max_num_particles)
z=ti.field(dtype=ti.f32,shape=max_num_particles)
...
@ti.kernel
def conjugate_gradient_iterate():
    for i in range(num_particles[None]):
        z[i] = r[i].dot(r[i])
        q[i] = ti.Vector([0,0])
        for j in range(num_particles[None]):
...
def substepimplicit(b:ti.f32):#使用implicit
    update_M_matrix()
    update_J_matrix()
    update_A_matrix(b)
    update_F_vector()
    ...
    conjugate_gradient_init()

    for i in range(1):
        conjugate_gradient_iterate()
